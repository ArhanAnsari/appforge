...
AppForge v0.1.1-alpha has a clean, modular structure organized by function. This document explains each folder and its purpose.
...
│   ├── core/                  ✅ Event, refresh, and output infrastructure (ACTIVE)
...
- `appwriteClientService.ts` - Project-scoped Appwrite client factories
- `projectStorageService.ts` - Project storage + secure credentials

**Status**: Production-ready
**Responsibility**:

- Create isolated Appwrite SDK clients per project
- Resolve project-specific services on demand
...
**Purpose**: Refresh orchestration, event routing, and output logging

**Files**:

- `refresh/refreshManager.ts` - Debounced scoped refresh coordination
- `events/eventBus.ts` - Internal event dispatching
- `output/outputChannel.ts` - Structured extension logging

**Status**: Production-ready
**Responsibility**:

- Coordinate UI refresh timing
- Broadcast internal events safely
- Centralize structured output messages
...
├── Project-scoped factory pattern
├── getProjectScopedClient()
├── createDatabasesService()
...
- **AppwriteClientService**: Project-scoped SDK integration
...
| `src/core/`      | ✅ Active       | Refresh/logging  | YES         |
...
**Bottom Line**: This is a production-ready structure for v0.1.1-alpha. Keep empty folders for Phase 2+. Add new folders only when needed.
